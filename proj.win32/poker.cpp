    ...
    int       nCmdShow)
{
    ...
    return PokerPlayer::createAndRun();
}

int PokerPlayer::createAndRun(void)
{
    PokerPlayer *host = PokerPlayer::sharedInstance();
    int ret = host->run();
    PokerPlayer::purgeSharedInstance();
    return ret;
}

PokerPlayer *PokerPlayer::s_sharedInstance = NULL;

PokerPlayer *PokerPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new PokerPlayer();
    }
    return s_sharedInstance;
}

void PokerPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
int PokerPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "poker-player");
...
void PokerPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void PokerPlayer::createViewMenu(void)
{
...
void PokerPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void PokerPlayer::updateWindowTitle(void)
{
}

void PokerPlayer::relaunch(void)
{
...
void PokerPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void PokerPlayer::onFileOpenProject(void)
{
...
void PokerPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"PokerPlayer");
...
void PokerPlayer::onFileProjectConfig(void)
{
...
void PokerPlayer::onFileRelaunch(void)
{
    relaunch();
}

void PokerPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void PokerPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void PokerPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void PokerPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void PokerPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT PokerPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    PokerPlayer *host = PokerPlayer::sharedInstance();
...
INT_PTR CALLBACK PokerPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
